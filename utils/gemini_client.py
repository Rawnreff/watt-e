                ...
                """Gemini client + deterministic fallback for consumption prediction.

                This module provides a GeminiClient class with a `predict` method that:
                - builds a short textual history prompt from up to 6 previous records
                - computes a deterministic prediction (WMA + simple linear trend)
                - optionally calls an LLM endpoint to refine the prediction
                - blends LLM and deterministic predictions and returns a structured dict

                If the LLM call fails or returns non-numeric results, a robust fallback
                based on historical averages is returned.
                """
                    ...
                    def _parse_number(value: Any) -> Optional[float]:
                        ...
                        class GeminiClient:
                            ...
                            def _parse_number(value: Any) -> Optional[float]:
                                if value is None:
                                    return None
                                """Clean Gemini client implementation."""
                                ...
                                """
                                    ...
                                    def get_fallback_prediction(self, history_list: Optional[List[Dict[str, Any]]], golongan_tarif: Optional[Any]) -> Dict[str, Any]:
                                        vals: List[float] = []
                                        if isinstance(history_list, (list, tuple)):
                                            for h in history_list:
                                                if not isinstance(h, dict):
                                        ...
                                        base_multiplier = 1.1
                                        if vals:
                                            predicted_kwh = sum(vals) / len(vals) * base_multiplier
                                        else:
                                            predicted_kwh = 0.0
